...
class Classification(object):
    ...
    def data_prepration(self):
        ...
        Args:
            train_path: Path to folder containing the dataset
        ...
        if self.image_size == False or self.image_size == None:
            Training_Input_shape, num_channels, _ = get_input_image_sizes(self.iterations_over_dataset,
                                                                          self.path, self.use_algorithm)
        else:
        ...
        return [Training_Input_shape, num_channels, network_input_size,
                train_path, unlabel_path, train_image_files, val_image_files,
                unlabel_image_files, steps_per_epoch]

    def data_generator(self, data_path, unlabel_path,
                       Training_Input_shape, num_channels, train_image_files, unlabel_image_files, val_image_files):
        '''
        Prepare data in Training and Validation sets
        Args:
            ...
            val_image_files: List of validation image files
        returns:
            A Training and Validation data generator
        '''
        assert isinstance(self.num_classes, int), \
            logging.error("Number of classes has not been set. You net to set num_classes!")
        
        TrainingDataGenerator = training_data_generator_classification_gen(Training_Input_shape,
                                                                           self.batchsize,
                                                                           num_channels,
                                                                           self.num_classes,
                                                                           train_image_files,
    ...
    def load_model(self, network_input_size):
        '''
        Build a ResNet50 model and initialize it with pretrained or random weights
        Args:
    ...
    def train_model(self, model, TrainingDataGenerator, ValidationDataGenerator, steps_per_epoch, val_image_files):
        ...
        Args:
            ...
            TrainingDataGenerator: The training data generator
    ...
    def uncertainty_prediction(self,
        ...
        for i in range(0, 20):
            ...
            testGene = testGenerator(Training_Input_shape, self.path, num_channels, test_image_files,
                                     self.use_algorithm)
    ...
    def run(self):
        ...
        unlabel_path = data_prepration_results[4]
        train_image_files = data_prepration_results[5]
        val_image_files = data_prepration_results[6]
        unlabel_image_files = data_prepration_results[7]
        steps_per_epoch = data_prepration_results[8]
        if self.iterations_over_dataset != 0:
            TrainingDataGenerator, ValidationDataGenerator = self.data_generator(data_path, unlabel_path,
                                                                                 Training_Input_shape,
                                                                                 num_channels,
                                                                                 train_image_files,
                                                                                 unlabel_image_files,
